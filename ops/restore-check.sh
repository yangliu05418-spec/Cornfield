...
runtime_privileges="$(docker exec "${container}" psql -X --no-password -U studio_bootstrap -d studio -Atc \
  "SELECT
      ...
      AND NOT has_table_privilege('studio_api','assets','INSERT')
      ...
      AND has_column_privilege('studio_api','assets','owner_user_id','INSERT')
      AND has_column_privilege('studio_api','assets','blur_data_url','INSERT')
      AND has_column_privilege('studio_api','layer_set_items','published_asset_id','UPDATE')
      AND NOT has_column_privilege('studio_api','layer_set_items','asset_id','UPDATE')
