...
`ops/prometheus-rules.yml` 已配置：target down、宿主文件系统 `<20%`/`<10%`、上传预留容量接近全局上限（预留字节大于 768 MiB 或 session 大于 24）、Worker 心跳缺失/超过 45 秒、最老队列等待超过 120 秒持续 5 分钟、`submission_uncertain`、Provider breaker 和 5 分钟内重复 Provider 失败。
...
安装 `ops/systemd/internal-image-studio-backup.{service,timer}`，执行 `systemctl enable --now internal-image-studio-backup.timer`。`STUDIO_ROOT`、`DATA_ROOT` 与 `BACKUP_STAGE` 都必须是绝对路径，且 `BACKUP_STAGE` 不能是文件系统根目录；脚本会在任何停止服务或写入前拒绝危险配置。随后脚本先记录 Worker 是否运行并将其停止，再在 PostgreSQL 容器内通过本地 socket 以 `studio_bootstrap` 生成保留 owner/ACL 的 custom-format dump；Worker 停止期间 API 仍可执行短数据库事务并接收 quarantine 上传，但不会提升、覆盖或删除正式资产，因此 dump 中的正式资产引用与随后 Restic 读取的不可变资产树保持一致，快照中多出的 quarantine 文件只会成为可安全清理的临时数据。脚本把 dump、实际 `DATA_ROOT`、模型/部署配置、`.env` 和 secret 目录送入加密 Restic 仓库；无论 dump/Restic 成功或失败，trap 都会恢复原本运行的 Worker。快照成功封存后立即恢复 Worker，再在不延长生成停机窗口的情况下执行 `forget --prune`；策略为 7 个日快照、4 个周快照。

Restic 仓库必须位于异机/对象存储；同机快照不能抵御磁盘和整机故障。每天检查 timer 与最新 snapshot，任何备份失败都应在下次发布前恢复。

## 恢复演练

安装 `ops/systemd/internal-image-studio-restore-check.{service,timer}` 并启用 timer。`RESTORE_CHECK_ROOT` 必须是预先容量规划的专用恢复盘，且与系统根目录和生产 `DATA_ROOT` 位于不同文件系统；脚本会拒绝使用 `/tmp`、系统盘或生产资产盘。它每月把最新快照恢复到该盘的临时目录，在限制为 4GiB 内存、2 CPU 的一次性 PostgreSQL 容器中使用同一临时磁盘目录，由 `studio_bootstrap` 预建 `studio_owner`、`studio_api`、`studio_worker`，把数据库/Schema owner 设为 `studio_owner`，再由非超管的 `studio_owner` 以 `--exit-on-error --single-transaction` 原样恢复 owner 与 ACL；这同时证明 archive 不依赖 superuser 才能恢复。演练会验证三个运行角色均非超管、数据库/Schema/对象 owner、API/Worker 的正负权限、所有未清理资产与 staged output 的文件引用均存在，并随机抽样核对 SHA-256；它不挂载或修改生产 volume，结束后只删除专用盘内由本次任务创建的 `cornfield-restore-*` 目录：
